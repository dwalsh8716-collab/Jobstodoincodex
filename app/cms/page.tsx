...
export default async function CmsPage({ searchParams }: Props) {
  ...
  const loggedIn = await isCmsSessionValid(cookieStore.get(CMS_SESSION_COOKIE)?.value);
  const redirectTo = params?.next && params.next.startsWith("/") && !params.next.startsWith("//") ? params.next : "/studio";
    ...
    <section className="section cms-shell">
      <div className="container cms-layout">
        <div className="cms-panel cms-intro">
          ...
          <p className="lede">
            Log in here, open the CMS, and update the parts of the website that need to move with the business.
          </p>
        ...
        <div className="cms-panel cms-login-card">
          {loggedIn ? (
            <>
              ...
              <p>
                The front door is open. Go into the CMS Studio to edit content, add media, review jobs or prepare
                launch updates.
              </p>
            ...
            <>
              ...
              <p>
                This is a simple site-level gate for the preview and live website. The connected Sanity account then
                handles the full editor workspace.
              </p>
              ...
              {params?.error ? <div className="cms-alert">Those details did not match. Try again.</div> : null}

              <form className="cms-login-form" action="/api/cms/login" method="post">
                <input type="hidden" name="redirectTo" value={redirectTo} />
                <label>
                  <span>Username</span>
                  <input name="username" type="text" autoComplete="username" defaultValue={getCmsUsername()} required />
                </label>
                <label>
                  <span>Password</span>
